...
export async function runWorkflow({
  ...
  void taskQueue;
  return {
    workflowId: `flue:${encodeWorkflowRequest(workflowName, input)}`,
...
export async function executeWorkflow(
  ...
  try {
    const result = await executeFlueWorkflow(workflowName, input);
...
export async function getWorkflowResult({
  ...
  try {
    if (!workflowId.startsWith("flue:")) {
      throw new Error(`Unsupported workflow id: ${workflowId}`);
    }
    const { workflowName, input } = decodeWorkflowRequest(workflowId.slice(5));
    const result = await executeFlueWorkflow(workflowName, input);
    if (process.env.NODE_ENV === "development") {
      console.log(
